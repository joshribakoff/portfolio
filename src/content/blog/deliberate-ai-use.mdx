...
There's a middle ground. Instead of agents fighting in chaos, run it like an organization where each agent operates in its own git worktree. Tools like [John Lindquist's worktree-cli](https://github.com/johnlindquist/worktree-cli) and [Bearing](https://bearing.dev) let you spin up isolated branches for parallel work. Bearing even has an experimental feature (homage to Gastown) that spawns recursive Claude Code instances to summarize branches with Haiku. Uses JSONL files instead of SQLite, scales to over 100 worktrees locally. No orchestrator, no chaos - just isolated slices of work.
