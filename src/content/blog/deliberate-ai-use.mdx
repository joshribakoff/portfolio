...
import bearingTui from "../../assets/blog/deliberate-ai-use/bearing-tui.svg";
...
There's a middle ground. Instead of agents fighting in chaos, run it like an organization where each agent operates in its own git worktree. Tools like [John Lindquist's worktree-cli](https://github.com/johnlindquist/worktree-cli) and [Bearing](https://bearing.dev) let you spin up isolated branches for parallel work.

<Image src={bearingTui} alt="Bearing TUI - Terminal interface for managing git worktrees" width={800} />

I'm not spinning up 30 agents for 30 worktrees. I'm staying in the loop - deciding when to background work, when to clear context, [steering](https://fightingwithai.com/workflow-guardrails/steering) interactively using the patterns I've documented. Concurrency without contention. I can have a hundred things I'm working on without the overhead of switching between them or worrying about conflicts in real time. When conflicts arise later, I just run another Claude instance to integrate the branches.
